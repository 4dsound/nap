...
//    cloudValue = 0.5 + 0.5*cloudValue;
    ...
    out_Color = vec4(ubo.color, distanceValue * distanceValue * pow(pass_Level, 0.75) * 0.9 * (1. - 0.5 * cloudValue)) +   vec4(ubo.color + vec3(0.2, 0.2, 0.2), extraValue * pow(pass_Level, 0.75) * 0.1);
;
}
