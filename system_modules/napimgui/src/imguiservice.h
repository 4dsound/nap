...
namespace nap
{
	...
	private:
		...
		struct NAPAPI GUIContext
		{
			...
			std::array<bool,4> mModPressed					= { false };					///< If the ctrl (0), alt (1) or shift (2) modifier key is pressed
			std::array<bool,4> mModRelease					= { false };					///< If the ctrl (0), alt (1) or shift (2) modifier key is released
